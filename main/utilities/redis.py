...
def push_to_redis(u_id, fname, lname, photo_path):
    ...
    my_server.set('user:%d' % u_id, json.dumps(user_info))
...
def load_feature_to_redis(visitors):
    for v_id, visitor in visitors.items():
        # Create user and save it into redis db
        user_info = {
            'id': int(v_id),
            'name': '%s %s' % (visitor.fname, visitor.lname),
            'photo_path': os.path.abspath(visitor.photo_path),
            'feature': visitor.face_feature.features
        }
